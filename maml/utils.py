...
def fast_adapt(batch, learner, loss, adaptation_steps, device):
    ...
    for step in range(adaptation_steps):
        adaptation_error = loss(learner(support_X), support_y)
        learner.adapt(adaptation_error)

    # Evaluate the adapted model
    predictions = learner(query_X)
    evaluation_error = loss(predictions, query_y)
    evaluation_accuracy = accuracy(predictions, query_y)
